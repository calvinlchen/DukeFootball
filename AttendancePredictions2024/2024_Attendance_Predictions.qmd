...
Below are models that combine the variables from Section 1 with the variables explored in the model above to create models with the highest-possible adjusted r-squared values:
...
late_jan_pred_model_data |>
  ...
  geom_point() +
  geom_errorbar(aes(ymin = AttPct - SdErrPct, ymax = AttPct + SdErrPct)) +
  ...
  geom_hline(yintercept = 62.07187, color = "blue",
             linetype = "dashed", size = 1,  alpha = 0.5) +
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  annotate("text", x = Inf, y = -Inf, hjust = 1, vjust = 0,
           label = "Predictive model and visualizations developed by Calvin Chen",
